---
title: "Introduction"
format:
  html:
    out-width: 100%
    code-fold: true
    code-summary: "Show source code"
    toc: true
    toc-title: Innhold
    embed-resources: true
editor: visual
engine: knitr
number-sections: true
theme: 
  - united
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{introduction}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
options(rmarkdown.html_vignette.check_title = FALSE)
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{css, echo=FALSE}
img {
    border: white;
}
```

This vignette provides examples on how to use different plotting functions.

```{r, eval=TRUE, message=FALSE}
library(ggplot2)
library(rrplot2)
```

# Bar chart

```{r, out.width='100%'}
mtcars |>
  ggplot(aes(x = factor(cyl),
             fill = factor(am))) +
  theme_RR() +
  geom_bar_RR() +
  labs(title = "Dette er et søylediagram",
       subtitle = "Fordelt på undergrupper",
       x = "Gruppe",
       y = "Antall",
       fill = "Kategori") +
  scale_fill_RR()
```

# Scatter plot (discrete palette)

## One plot

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
iris |>
  ggplot(aes(x = Sepal.Length,
             y = Sepal.Width,
             color = Species)) +
  theme_RR() +
  geom_point_RR() +
  labs(title = "Scatterplot",
       subtitle = "Dette er en beskrivelse som gir leseren mer informasjon.",
       x = "X-variabel",
       y = "Y-variabel") +
  scale_color_RR()

```

## Small multiples

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
iris |> 
  ggplot(aes(x = Sepal.Length,
             y = Sepal.Width)) +
  geom_point(aes(color = Species)) +
  facet_wrap(~ Species,
             nrow = 2) +
  theme_RR() +
  scale_color_RR() +
  labs(x = "X-variabel",
       y = "Y-variabel",
       color = "Gruppe",
       title = "Oppdelt plot",
       subtitle = "Med flere underkategorier")  +
  theme(panel.background = ggplot2::element_rect(fill = "#f9f9f9", linetype = 0))
```


# Scatter plot (continuous palette)

## Red

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
mtcars |>
  ggplot(aes(x = hp,
             y = mpg,
             color = cyl)) +
  theme_RR() +
  geom_point_RR() +
  labs(x = "X-variabel",
       y = "Y-variabel",
       color = "Farge") +
  scale_color_continuous_RR_red()

```

## Blue

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
mtcars |>
  ggplot(aes(x = hp,
             y = mpg,
             color = cyl)) +
  theme_RR() +
  geom_point_RR() +
  labs(x = "X-variabel",
       y = "Y-variabel",
       color = "Farge") +
  scale_color_continuous_RR_blue()
```

# Density plot

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
# Generate example data
set.seed(123)
data <- data.frame(
  Group = rep(c("Group1", "Group2"), each = 200),
  Value = c(rnorm(200, mean = 0, sd = 1), rnorm(200, mean = 2, sd = 1)))

# Create density plot
data |>
  ggplot(aes(x = Value, fill = Group)) +
  theme_RR() +
  geom_density_RR(alpha = 0.5) +
  labs(x = "Value", y = "Density",
       fill = "Group",
       title = "Density plot",
       subtitle = "Med to undergrupper") +
  scale_fill_RR()
```

# Line chart (simple)

## One plot

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
data("economics_long")
economics_long |> 
  ggplot(aes(x = date, y = value, color = variable)) +
  geom_line_RR(linewidth = 0.8) +
  labs(title = "Linjediagram med utvikling over tid",
       subtitle = "For en rekke økonomiske indikatorer",
       x = "År",
       y = "Verdi",
       color = "Indikator") +
  theme_RR() +
  scale_color_RR()
```

## Small multiples

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=8}
economics_long |> 
  ggplot(aes(x = date, y = value, color = variable)) +
  geom_line_RR(linewidth = 0.8) +
  labs(title = "Linjediagram med utvikling over tid",
       subtitle = "For en rekke økonomiske indikatorer",
       x = "År",
       y = "Verdi",
       color = "Indikator") +
  theme_RR() +
  scale_color_RR() +
  facet_wrap(~variable, scales = "free",
             nrow = 3) +
  scale_y_continuous(limits = c(0, NA))  +
  theme(panel.background = ggplot2::element_rect(fill = "#f9f9f9", linetype = 0))
```

# Heat map

## Blue

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
# Dummy data
x <- LETTERS[1:20]
y <- paste0("var", seq(1,20))
data <- expand.grid(X=x, Y=y)
data$Z <- runif(400, 0, 5)
 
# Heatmap 
data |> 
  ggplot(aes(X, 
             Y, 
             fill= Z)) + 
  geom_tile() + 
  theme_RR() +
  scale_fill_continuous_RR_blue()
```

## Red

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
data |> 
  ggplot(aes(X, 
             Y, 
             fill= Z)) + 
  geom_tile() + 
  theme_RR() +
  scale_fill_continuous_RR_red()
```

# Survey data

## Blue likert color scale

```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
data <- data.frame(
  Question = rep(c("Spørsmål 1", "Spørsmål 2", "Spørsmål 3"), each = 5),
  Response = rep(c("Svært uenig", "Uenig", "Hverken eller", "Enig", "Svært enig"), times = 3),
  Count = c(5, 10, 20, 30, 35, 3, 8, 15, 25, 49, 2, 7, 18, 28, 45)
)

data$Response <- factor(data$Response,
                        levels = c("Svært uenig", "Uenig", "Hverken eller", "Enig", "Svært enig"))

 data |>
  ggplot(aes(x = Question, y = Count, fill = Response)) +
  geom_bar_RR(stat = "identity") +
  coord_flip() +
  labs(title = "Svar på spørsmål",
       subtitle = "Basert på en spørreundersøkelse som går til 100 tilfeldige personer.",
       fill = "Kategori",
       x = "Spørsmål", y = "Prosent") +
  theme_RR() +
  scale_fill_traffic_blue_RR()
```

## Green likert color scale
```{r, dev = "png", out.width='100%', fig.width=5, fig.height=3.5}
data |>
  ggplot(aes(x = Question, y = Count, fill = Response)) +
  geom_bar_RR(stat = "identity") +
  coord_flip() +
  labs(title = "Svar på spørsmål",
       subtitle = "Basert på en spørreundersøkelse som går til 100 tilfeldige personer.",
       fill = "Kategori",
       x = "Spørsmål", y = "Prosent") +
  theme_RR() +
  scale_fill_traffic_green_RR() 
```

